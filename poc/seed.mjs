 ...
 * with a reference build (#1) holding the Checkout + Signup funnels, a check
 * build (#2) where two checkout steps changed, and a "smooth/dashboard"
 * project with no reference build and Storybook-only metadata.
 *
 *   NODE_ENV=test node poc/seed.mjs > poc/seed-output.json
 *
 * Prints JSON: { rawToken, buildNumbers, diffIds } for the Playwright driver.
 */
