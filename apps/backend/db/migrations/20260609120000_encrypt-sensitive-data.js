...
function getKey() {
  // In production, refuse to run with the zero-key default: encrypting rows with
  // a publicly-known key would be a silent footgun. Locally / in CI we mirror the
  // config default in apps/backend/src/config/index.ts so runs without
  // ENCRYPTION_KEY derive the same key as the application.
  if (process.env.NODE_ENV === "production" && !process.env.ENCRYPTION_KEY) {
    throw new Error(
      "ENCRYPTION_KEY must be set to run this migration in production.",
    );
  }
  const hexKey =
