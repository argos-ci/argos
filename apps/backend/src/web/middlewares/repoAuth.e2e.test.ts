  ...
  describe("GitHub token", () => {
    ...
    it("returns 401 with unknown repository", async () => {
      ...
      await request(app)
        ...
        .expect((res) => {
          expect(res.text).toBe(
            `Argos Error: Project not found. Ensure a project exists in Argos (https://app.argos-ci.com) and restart your test after setup. Persisting issue? Consider adding 'ARGOS_TOKEN' to your CI environment variables. (token: "tokenless-github-${encodedToken}").`,
          );
        })
        .expect(401);
    });
