import "@/automation/actions/loader.js";
...
describe("automation/triggerAutomation", () => {
  ...
  describe("triggerAutomation", () => {
    ...
    it("should handle unknown action types gracefully", async () => {
      ...
      await expect(
        triggerAutomation(project.id, AutomationEvent.BuildCompleted, build),
      ).rejects.toThrow(
        "Invariant failed: [AutomationEngine] Unsupported action type: unknown_action_type. Action not found in registry.",
      );
