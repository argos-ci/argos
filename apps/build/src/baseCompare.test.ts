...
function nockGithub(branch: string) {
...
describe("#baseCompare", () => {
  ...
  describe("simple cases", () => {
    ...
    // Can't be tested because nock is not compatible with the new GitHub API (using fetch)
    it.skip("should work with a non rebased PR", async () => {
  ...
  describe("master", () => {
    ...
    it("should handle master commits correctly", async () => {
      await factory.create<ScreenshotBucket>("ScreenshotBucket", {
        branch: "master",
  ...
  it("should try every commits available", async () => {
    ...
    const bucket1 = await factory.create<ScreenshotBucket>("ScreenshotBucket", {
      commit: "d2e624599bff1f9103bca64848fe17768da9cfa6",
      repositoryId: repository.id,
      branch: "master",
    });
    const bucket2 = await factory.create<ScreenshotBucket>("ScreenshotBucket", {
      commit: "e97e6d9054ac1b4f6f9ef55ff3d7ed8cc18394bd",
      repositoryId: repository.id,
      branch: "x",
    });
